...
public class AlterarVoosController implements Initializable{
	@FXML
	...
	@FXML
	public void alterarVoo(ActionEvent event) {
		...
		if(!ciu.equals("") && !horaS.equals("") && !horaC.equals("") && !minutoS.equals("") && !minutoC.equals("")
			...
			try{
				...
				ZonedDateTime dataSaidaFuso = ZonedDateTime.of(saidaSemFuso, cO.getZoneId());
				ZonedDateTime dataChegadaFuso = ZonedDateTime.of(chegadaSemFuso, cD.getZoneId());
				
				Voos v = new Voos(codIU, cO, cD, aviao, dataSaidaFuso, dataChegadaFuso);

				Stage stage = (Stage) attBttn.getScene().getWindow();
				Fachada.getInstance().alterarVoo(indexAtt, v);
